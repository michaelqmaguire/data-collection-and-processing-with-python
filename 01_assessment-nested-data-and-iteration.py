...
# ------------------------------------------------- #
# Week 1 - Assessment: Nested Data and Iteration    #
# ------------------------------------------------- #
...
# This question doesn't make sense to me - Reversed test1 values to get correct answer, but 'hola' is in list L?
...
for k in nested_d.keys():
    for v in nested_d[k].keys():
        if v == 'USA':
            US_count.append(nested_d[k][v])
...
for lstofath in athletes:
    for athlete in lstofath:
        if 't' in athlete:
            t.append(athlete)
        else:
